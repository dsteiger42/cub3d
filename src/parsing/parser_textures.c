...
/*   Updated: 2025/09/15 10:25:11 by samuel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_wall(t_data *d, t_img *s, int x, t_ray *ray, t_texture *tex)
{
	double	step;
	double	tex_pos;
	int		y;
	...
	tex_pos = (ray->draw_start - d->screen_h / 2 + ray->line_height / 2) * step;
	y = ray->draw_start;
	while (y <= ray->draw_end)
	{
		tex_y = (int)tex_pos & (tex->height - 1);
		put_pixel(s, x, y, tex->data[tex_y * tex->width + tex_x]);
		tex_pos += step;
		y++;
	}
}
...
void	draw_column(t_data *data, t_img *screen, int x, t_ray *ray)
{
	...
	draw_wall(data, screen, x, ray, tex);
